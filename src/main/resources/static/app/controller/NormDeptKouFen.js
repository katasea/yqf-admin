...
var normId=null;
Ext.define('MyApp.controller.NormDeptKouFen', {
    extend: 'Ext.app.Controller',
    stores: ['NormDeptKouFen','NormTree'],
    models: ['NormDeptKouFen'],
    views:  ['normDeptKouFen.Main','normDeptKouFen.Tree'],
    init: function () {
    	...
    	this.control({
    		'actioncolumn#actioncolumnEditAndDel': {  
    			...
    			deleteclick: function(pa) {
                    ...
                    Ext.Msg.confirm("提示","确定删除<span style='color:red;font-weight:bold'>"+name+"</span>吗？不可恢复！",function(btn){
                    	if(btn=='yes') {
                    		mask.show();
                    		Ext.Ajax.request({
                			    url: contextPath + '/normDeptKouFen/del',
                			    params: {
                			        id   : id,
                			        name : name
                			    },
    	 	...
    	 	'normDeptKouFenMain button[action=refresh]': {
                click: this.refresh
            },
            'normDeptKouFenMain button[action=add]': {
                click: this.add
            },
            'normDeptKouFenMain button[action=print]': {
                click: this.print
            },
            'normDeptKouFenMain button[action=search]': {
                click: this.search
            },
            'normDeptKouFenAdd button[action=save]': {
                click: this.save
            },
            'normDeptKouFenAdd button[action=cancel]': {
                click: this.cancel
            },
            'normDeptKouFenMain':{
            	itemdblclick : this.gridDBClickListener
            },
            'normDeptKouFenMain combobox[name=zhiBiaoType]': {
                select:function(combo, records, op){
                	var val = records[0].get("dickey");
                	this.getNormTreeStore().load({params:{zhiBiaoType:val}});
                }
            },
        ...
        store = this.getNormDeptKouFenStore();
        //Get my store and add listener 
        this.getNormDeptKouFenStore().on('beforeload',function(store, operation, eOpts ){
    ...
    refresh : function(btn) {
    	this.getNormDeptKouFenStore().load({params:{start:0}});
    },
    add : function(btn){
        old = '';
   		father = null;id = null;
    ...
    treeItemClick:function(objtree, record, item, index, e, eOpts ){
        ...
        if(leaf) {
            this.getNormDeptKouFenStore().load();
        }
    },
    search:function(btn) {
    	this.refresh();
