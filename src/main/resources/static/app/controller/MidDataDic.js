var uuid = null;//定义一个全局变量id 有值就是修改无值就是新增
var store = null;//需要刷新哪个节点
var currentnodeid = '';
var old = '';
var father = null;//定义一个全局变量parentid用于记录是新增子节点还是顶级父节点
Ext.define('MyApp.controller.MidDataDic', {
    extend: 'Ext.app.Controller',
    stores: ['MidDataDic','MidDataDicParent'],
    models: ['MidDataDic','MidDataDicParent'],
    views:  ['midDataDic.Main','midDataDic.Add','midDataDicParent.Main','midDataDicParent.Add'],
    init: function () {
    	...
    	this.control({
    		'actioncolumn#actioncolumnEditAndDel': {  
    			...
    			deleteclick: function(pa) {
                    ...
                    Ext.Msg.confirm("提示","确定删除<span style='color:red;font-weight:bold'>"+name+"</span>吗？不可恢复！",function(btn){
                    	if(btn==='yes') {
                    		mask.show();
                    		Ext.Ajax.request({
                			    url: contextPath + '/midDataDic/del',
                			    params: {
                			        id   : id,
                			        name : name
                			    },
    	 	...
    	 	'midDataDicMain button[action=refresh]': {
                click: this.refresh
            },
            'midDataDicMain button[action=add]': {
                click: this.add
            },
            'midDataDicMain button[action=print]': {
                click: this.print
            },
            'midDataDicMain button[action=autoInitFromAccess]': {
                click: this.autoInitFromAccess
            },
            'midDataDicMain button[action=search]': {
                click: this.search
            },
            'midDataDicAdd button[action=save]': {
                click: this.save
            },
            'midDataDicAdd button[action=cancel]': {
                click: this.cancel
            },
            'midDataDicMain':{
        ...
        store = this.getMidDataDicStore();
        //Get my store and add listener 
        this.getMidDataDicStore().on('beforeload',function(store, operation, eOpts ){
    ...
    refresh : function(btn) {
    	this.getMidDataDicStore().load({params:{start:0}});
    },
    add : function(btn){
        old = '';
   		father = null;uuid = null;
    ...
    edit:function(vid,record) {
		...
		node = null;
		win.down('midDataDicAdd').getForm().loadRecord(record);
        win.down('midDataDicAdd').down('textfield[id="id"]').setDisabled(true);
    },
    createAddWin:function(flag){
    	var iconCls = '';var title = '';if(flag === 'add') {iconCls = 'Add';title = '新增'}else if(flag === 'edit'){iconCls='Edit';title='修改'}
    	var form = Ext.widget('midDataDicAdd');
    	var window = Ext.create('Ext.window.Window', {
		    title:title,iconCls:iconCls,height: 350,
		    frame : true,border : 0,autoScroll:true,width: 450,minWidth:350,
    ...
    autoInitFromAccess:function(btn) {
        ...
        Ext.Msg.confirm('提示','确定初始化内置数据吗?操作不可逆，会删除已存在数据!确定操作吗?',function(btn) {
            if(btn==='yes') {mask.show();
                Ext.Ajax.request({
                    url: contextPath + '/midDataDic/autoInitFromAccess',params: {},
                    success: function(response){
    ...
    save:function(btn){
        ...
        if (form.isValid()) {
            ...
            form.submit({
            	params:{father:father,uuid:uuid},
