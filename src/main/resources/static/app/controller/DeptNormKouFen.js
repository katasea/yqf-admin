...
Ext.define('MyApp.controller.DeptNormKouFen', {
    extend: 'Ext.app.Controller',
    stores: ['DeptNormKouFen','DeptTree'],
    models: ['DeptNormKouFen'],
    views:  ['deptNormKouFen.Main','deptNormKouFen.Tree'],
    init: function () {
    	...
    	this.control({
    		'actioncolumn#actioncolumnEditAndDel': {  
    	 	...
    	 	'deptNormKouFenMain button[action=refresh]': {
                click: this.refresh
            },
            'deptNormKouFenMain button[action=add]': {
                click: this.add
            },
            'deptNormKouFenMain button[action=print]': {
                click: this.print
            },
            'deptNormKouFenMain button[action=search]': {
                click: this.search
            },
            'deptNormKouFenAdd button[action=save]': {
                click: this.save
            },
            'deptNormKouFenAdd button[action=cancel]': {
                click: this.cancel
            },
            'deptNormKouFenMain':{
        ...
        store = this.getDeptNormKouFenStore();
        //Get my store and add listener 
        this.getDeptNormKouFenStore().on('beforeload',function(store, operation, eOpts ){
    ...
    refresh : function(btn) {
    	this.getDeptNormKouFenStore().load({params:{start:0}});
    },
    add : function(btn){
        old = '';
   		father = null;id = null;
    ...
    treeItemClick:function(objtree, record, item, index, e, eOpts ){
        ...
        if(leaf) {
            this.getDeptNormKouFenStore().load({param:{start:0}});
            Ext.getCmp('vpcViewport').down('deptNormKouFenMain').down('button[action="search"]').setDisabled(false);
        }
    },
    search:function(btn) {
    	this.refresh();
